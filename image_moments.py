...
def calc_central_moments(matrix:np.ndarray) -> dict:
    ...
    if m00 == 0:
        return {k: 0.0 for k in ["eta_00","eta_01","eta_10","eta_11","eta_02",
                                 "eta_20","eta_21","eta_12","eta_03","eta_30"]}

    m10 = np.sum(x_coords * matrix)
    m01 = np.sum(y_coords * matrix)
    ...
    return {
        ...
        "eta_01": get_eta(0, 1), # Debería ser ~0.0
        "eta_11": get_eta(1, 1),
        "eta_02": get_eta(0, 2),
        "eta_20": get_eta(2, 0),
        "eta_21": get_eta(2, 1),
